...
double power_iteration_gpu(double **A, double *x, int size,double tol){
	printf("For Power iteration GPU\n");
	int maxiter = 100; // Initialize max error
	double lambda_old = 0.0;
	double lambda_new = 0.0;
	double *x_new = (double *) calloc(size, sizeof(double));
...
void jacobi_gpu(double **A, int size,double tol){
	printf("Jacobi method GPU\n");
	int maxiter = 2000;
	for(int iter=0; iter<maxiter;iter++){
		double max = 0.0;
		int a = 0;
...
int main(int argc, char *argv[]) {
	...
	if(algo==1){
		...
		getWallTime(&start);
		printf("YES!!!!!!\n");
		double lambda_omp = power_iteration_gpu(B,x,row,tol);
		getWallTime(&end);
		printf("Eigen value = %lf\n",lambda_omp);
		printExecutionTime(start, end);
		free(A);
		...
		start, end = 0.0;
		getWallTime(&start);
		jacobi_gpu(B,row,tol);
		getWallTime(&end);
		printExecutionTime(start, end);
		//print_array(B,row,row);
		free(A);
		free(B);
	}
